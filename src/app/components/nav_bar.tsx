...
const Navbar = () => {
  ...
  return (
    <header className="bg-black bg-opacity-90 py-4 px-6 flex justify-between items-center sticky top-0 z-50">
      ...
      >
        <i
          className={`fa ${isMenuOpen ? "fa-times" : "fa-bars"} text-2xl`} // Fixed string interpolation
        ></i>
      </button>

      <nav
        className={`${isMenuOpen ? "block" : "hidden"} absolute top-full left-0 w-full bg-black bg-opacity-90 md:relative md:flex md:space-x-6 md:w-auto md:top-auto md:left-auto`}
      >
        <a href="/Movies" className="block px-4 py-2 hover:text-purple-400 md:px-0">
          MOVIES
        </a>
        <a href="/MyTickets" className="block px-4 py-2 hover:text-purple-400 md:px-0">
          MY TICKETS
        </a>
